# Claude Cowork + Skills Content Implementation Plan

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** Add eight `Claude ·` cheat-sheet sections (full Cowork 40 + distilled Skills guide) to the existing offline browser via a new `ClaudeContent` module.

**Architecture:** Mirror `OpenClawContent`: static `[CheatSection]` in `ClaudeContent.swift`, append in `CheatSheetContent.sections`. No UI or Builder changes. Source PDFs stay outside the repo; copy uses `[bracket]` placeholders.

**Tech Stack:** Swift Package, SwiftUI macOS 14+, XCTest (`swift test`)

## Global Constraints

- Placement: same Cheat Sheet browser as Codex + OpenClaw (no new tab)
- File: `Sources/CodexCheatSheetCore/Models/ClaudeContent.swift`
- Wiring: `codexSections + OpenClawContent.sections + ClaudeContent.sections`
- Eight section titles must start with `Claude ·`
- Do not vendor PDFs; do not redesign views; do not add Builder templates in this pass
- Bracket placeholders only (`[Downloads]`, `[topic]`, etc.) — no personal absolute paths
- Do not commit unless the user asks (skip commit steps during execution unless requested)
- Prefer edit over inventing new top-level folders
...
| Path | Responsibility |
|------|----------------|
| `Sources/CodexCheatSheetCore/Models/ClaudeContent.swift` | All eight Claude sections (tables + copy-ready templates) |
| `Sources/CodexCheatSheetCore/Models/CheatSheetContent.swift` | Append `ClaudeContent.sections` to public `sections` |
| `Tests/CodexCheatSheetTests/BuilderTemplateAssembleTests.swift` | Smoke tests for Claude section count / titles / template coverage |
| `README.md` | Note Claude content lives in `ClaudeContent` |
| `AGENTS.md` | Note browser content includes Claude alongside Codex + OpenClaw |

**Spec:** `docs/superpowers/specs/2026-07-30-claude-cowork-skills-content-design.md`

---

### Task 1: Failing Claude smoke tests

**Files:**
- Modify: `Tests/CodexCheatSheetTests/BuilderTemplateAssembleTests.swift` (extend `ContentSmokeTests`)

**Interfaces:**
- Consumes: `CheatSheetContent.sections`, `CheatSection.title`, `CheatSection.templates`
- Produces: two failing tests that lock the Claude contract before content lands

- [ ] **Step 1: Write the failing tests**

Append inside `ContentSmokeTests` (keep existing OpenClaw tests intact):
...
```bash
swift test --filter ContentSmokeTests.testClaudeSectionsPresent
swift test --filter ContentSmokeTests.testClaudeCoworkWorkflowTemplatesPresent
```

Expected: FAIL (Claude sections missing / counts wrong).

- [ ] **Step 3: Commit** (only if user asked)

```bash
git add Tests/CodexCheatSheetTests/BuilderTemplateAssembleTests.swift
git commit -m "test: require Claude Cowork and Skills cheat-sheet sections"
```

---

### Task 2: Create `ClaudeContent` and wire it in

**Files:**
- Create: `Sources/CodexCheatSheetCore/Models/ClaudeContent.swift`
- Modify: `Sources/CodexCheatSheetCore/Models/CheatSheetContent.swift` (sections array only)

**Interfaces:**
- Consumes: `CheatSection`, `PatternRow`, `PromptTemplate` from `CheatSheetModels.swift`
- Produces: `enum ClaudeContent { static let sections: [CheatSection] }` with exactly the eight titles from Task 1

- [ ] **Step 1: Create `ClaudeContent.swift`**

Create `Sources/CodexCheatSheetCore/Models/ClaudeContent.swift` with the full content below (single file; SPM picks it up automatically — no `Package.swift` change).
...
- [ ] **Step 2: Wire into `CheatSheetContent`**

In `Sources/CodexCheatSheetCore/Models/CheatSheetContent.swift`, change:

```swift
static let sections: [CheatSection] = codexSections + OpenClawContent.sections
```
...
If `OpenClawContent` is missing in the working tree for any reason, keep Codex + Claude only after confirming OpenClaw status — do not delete OpenClaw wiring that already exists.
...
```bash
swift test --filter ContentSmokeTests
```

Expected: PASS for `testClaudeSectionsPresent` and `testClaudeCoworkWorkflowTemplatesPresent` (and existing smoke tests).
...
```bash
git add Sources/CodexCheatSheetCore/Models/ClaudeContent.swift \
  Sources/CodexCheatSheetCore/Models/CheatSheetContent.swift \
  Tests/CodexCheatSheetTests/BuilderTemplateAssembleTests.swift
git commit -m "Add Claude Cowork and Skills sections to the cheat sheet browser"
```

---

### Task 3: Document content ownership
...
**Interfaces:**
- Consumes: none
- Produces: docs that name `ClaudeContent` alongside Codex + OpenClaw

- [ ] **Step 1: Update README**

In **Where things live**, change the Core bullet to mention Claude:

```markdown
- `Sources/CodexCheatSheetCore/` — models, static content (Codex + OpenClaw + Claude), and SwiftUI views (library)
```

In **Content ownership**, add after the Browser bullet:
...
In **Content ownership**, replace the browser line block with:
...
- [ ] **Step 3: Re-run tests (docs-only; sanity)**
...
git commit -m "Document Claude content ownership in README and AGENTS"
```
...
|------------------|------|
| `ClaudeContent.swift` OpenClaw-style module | Task 2 |
| Wire into `CheatSheetContent.sections` | Task 2 |
| Eight `Claude ·` sections per map | Tasks 1–2 |
| All 40 Cowork items (table and/or templates) | Task 2 (10 slash rows + 8+8+8+6 templates) |
| Distilled Skills fundamentals / authoring / testing & patterns | Task 2 |
| Bracket placeholders; no PDFs in repo | Task 2 conventions |
| No new tab / no Builder this pass | Global Constraints |
| Smoke tests | Task 1 |
| README + AGENTS note | Task 3 |
| `swift test` passes | Tasks 2–3 |

No placeholders remaining. Title strings in tests match `ClaudeContent` exactly. Template counts: File 8, Connector 8, Document 8, Scheduled 6, Slash rows 10, Skill Authoring templates non-empty.
